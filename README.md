...
- **梦幻装车单**：选择图鉴车型、涂装、变速系统、轮组与轮胎，补充车把、坐垫、脚踏与小件；勾选备齐进度。重量按各行定义填写，未知项不视作零；预算统一为人民币。车型图片表示灵感平台，不伪装成更换配件后的照片。
...
- **海报与分享**：浏览器生成 1080 × 1600 PNG，一键发起下载，并保留预览、下载链接及系统文件分享（浏览器支持时）。可复制完整配置链接；链接包含名称、配件、输入重量和预算，不包含备齐进度。海报使用官方整车原图并保留署名，不能当作实际装配照片。
...
- **结构观察室**：Ratchet EXP 36 齿端面啮合与轮圈胎圈截面两个独立研究模型，可拖拽、缩放、点选、分解，支持动态啮合、隐藏外壳及截面正视。齿数、轮圈内宽与框高有官方依据；其余尺寸、制造细节和受力过程不作为原厂 CAD 或仿真数据。这与车型详情中的近似整车 3D 分开呈现。
